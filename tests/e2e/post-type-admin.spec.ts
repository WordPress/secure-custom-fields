...
test.describe( 'Post Type Creation', () => {
	...
	test( 'should create hierarchical post type with taxonomy and field group', async ( {
		...
		await admin.visitAdminPage( 'edit.php', `post_type=${ SCF_TAXONOMY_SLUG }` );
		await page.locator( 'a.acf-btn:has-text("Add New")' ).click();

		await page.fill( '#acf_taxonomy-labels-name', INTEGRATION_TAXONOMY_NAME );
		await page.fill( '#acf_taxonomy-labels-singular_name', INTEGRATION_TAXONOMY_SINGULAR );
		...
		await admin.visitAdminPage( 'edit.php', `post_type=${ SCF_POST_TYPE_SLUG }` );
		await page.locator( 'a.acf-btn:has-text("Add New")' ).click();

		await page.fill( '#acf_post_type-labels-name', INTEGRATION_POST_TYPE_NAME );
		await page.fill( '#acf_post_type-labels-singular_name', INTEGRATION_POST_TYPE_SINGULAR );
		await page.fill( '#acf_post_type-post_type', INTEGRATION_POST_TYPE_KEY );

		// Enable hierarchical (like pages)
		const hierarchicalToggle = page.locator( '.acf-field[data-name="hierarchical"] .acf-switch' );
		...
		const taxonomiesField = page.locator( '.acf-field[data-name="taxonomies"]' );
		await taxonomiesField.locator( '.select2-selection' ).click();
		await page.locator( `.select2-results__option:has-text("${ INTEGRATION_TAXONOMY_SINGULAR }")` ).click();
		...
		await expect(
			taxonomiesField.locator( `.select2-selection__choice:has-text("${ INTEGRATION_TAXONOMY_SINGULAR }")` )
		).toBeVisible();
		...
		const customPostTypeMenu = page.locator( '#adminmenu' ).getByRole( 'link', { name: INTEGRATION_POST_TYPE_NAME, exact: true } );
		await expect( customPostTypeMenu ).toBeVisible( { timeout: DEFAULT_TIMEOUT } );
		...
		await page.locator( `.wrap a:has-text("Add ${ INTEGRATION_POST_TYPE_SINGULAR }")` ).click();
		...
		const taxonomyPanel = page.getByRole( 'button', { name: INTEGRATION_TAXONOMY_NAME } );
		await expect( taxonomyPanel ).toBeVisible( { timeout: DEFAULT_TIMEOUT } );
		...
		await admin.visitAdminPage( 'edit.php', `post_type=${ SCF_FIELD_GROUP_SLUG }` );
		await page.locator( 'a.acf-btn:has-text("Add New")' ).click();
		...
		const paramSelect = page.locator( 'select[id^="acf_field_group-location-group_0-rule_0-param"]' );
		...
		const valueSelect = page.locator( 'select[id^="acf_field_group-location-group_0-rule_0-value"]' );
		await expect( valueSelect.locator( `option:has-text("${ INTEGRATION_POST_TYPE_SINGULAR }")` ) )
		...
		await admin.visitAdminPage( 'index.php', '' );
		await page.locator( '#adminmenu' ).getByRole( 'link', { name: INTEGRATION_POST_TYPE_NAME, exact: true } ).click();
		await page.locator( `.wrap a:has-text("Add ${ INTEGRATION_POST_TYPE_SINGULAR }")` ).click();

		await prepareBlockEditorForMetabox( page );
		await verifyFieldInMetabox( page, INTEGRATION_FIELD_GROUP_NAME, INTEGRATION_FIELD_LABEL );

		// SECTION 6: Verify all entities appear in their admin lists
		await admin.visitAdminPage( 'edit.php', `post_type=${ SCF_POST_TYPE_SLUG }` );
		await expect(
			page.locator( `#the-list a:has-text("${ INTEGRATION_POST_TYPE_NAME }")` )
		).toBeVisible();
		...
		await admin.visitAdminPage( 'edit.php', `post_type=${ SCF_FIELD_GROUP_SLUG }` );
		await expect(
			page.locator( `#the-list a:has-text("${ INTEGRATION_FIELD_GROUP_NAME }")` )
		).toBeVisible();
...
async function updatePostType( page, admin, newName, newSingular ) {
	...
	const postTypeLink = page.locator( `#the-list a.row-title:has-text("${ POST_TYPE_NAME }")` );
	await expect( postTypeLink ).toBeVisible( { timeout: DEFAULT_TIMEOUT } );
...
async function verifyPostTypeUpdated( page, admin, updatedName ) {
	...
	const updatedPostTypeLink = page.locator( `#the-list a:has-text("${ updatedName }")` );
	await expect( updatedPostTypeLink ).toBeVisible( { timeout: DEFAULT_TIMEOUT } );

	// Verify the original post type no longer exists
	const originalPostTypeLink = page.locator( `#the-list a:has-text("${ POST_TYPE_NAME }")` );
	...
	const postTypeCount = await page.locator( `#the-list a:has-text("${ updatedName }")` ).count();
...
async function deletePostType( page, admin, postTypeName = POST_TYPE_NAME ) {
	...
	const postTypeRow = page.locator( `tr.type-acf-post-type:has(a.row-title:text("${ postTypeName }"))` );
...
async function restorePostType( page, admin, postTypeName = POST_TYPE_NAME ) {
	await admin.visitAdminPage( 'edit.php', `post_status=trash&post_type=${ SCF_POST_TYPE_SLUG }` );

	// Find the trashed post type row
	const postTypeRow = page.locator( '#the-list tr', { hasText: postTypeName } );
	await expect( postTypeRow ).toBeVisible( { timeout: DEFAULT_TIMEOUT } );
	await postTypeRow.locator( 'th.check-column input[type="checkbox"]' ).check();
...
async function trashAllPostTypes( page, admin ) {
	...
	await emptyTrashButton.waitFor( { state: 'visible', timeout: DEFAULT_TIMEOUT } );
...
async function emptyTrashForPostType( page, admin, postType ) {
	...
	if ( await emptyTrashButton.isVisible( { timeout: 1000 } ).catch( () => false ) ) {
		await emptyTrashButton.click();
...
async function prepareBlockEditorForMetabox( page ) {
	...
	// Close pattern modal if it appears
	const patternModal = page.locator( '.components-modal__frame' );
	if ( await patternModal.isVisible() ) {
		await page.keyboard.press( 'Escape' );
		await expect( patternModal ).not.toBeVisible( { timeout: 3000 } );
	}
