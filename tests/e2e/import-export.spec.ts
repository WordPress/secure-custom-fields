...
const { PLUGIN_SLUG, deleteFieldGroups } = require( './field-helpers' );
...
test.describe( 'Import/Export', () => {
	...
	test.describe( 'JSON Export', () => {
		test( 'should export a field group as JSON', async ( {
			page,
			admin,
		} ) => {
			...
			await admin.visitAdminPage(
				'edit.php',
				'post_type=acf-field-group&page=acf-tools'
			);

			// Wait for export section to be visible
			const exportSection = page.locator(
				'.acf-postbox-header:has-text("Export")'
			);
			...
			const fieldGroupCheckbox = page
				.locator( `input[type="checkbox"][value*="group_"]` )
				.first();
			await expect( fieldGroupCheckbox ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
			...
			const exportButton = page.locator(
				'button[value="download"]:has-text("Export As JSON")'
			);
		...
		test( 'should display warning when no field groups selected for export', async ( {
			page,
			admin,
		} ) => {
			...
			const warningNotice = page
				.locator(
					'.acf-admin-notice.notice-warning, .notice.notice-warning'
				)
				.filter( { hasText: 'No field groups selected' } );
			await expect( warningNotice ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
		} );

		test( 'should export multiple field groups at once', async ( {
			page,
			admin,
		} ) => {
			...
			const fieldLabel1 = page.locator(
			...
			const fieldLabel2 = page.locator(
				'input[id^="acf_fields-field_"][id$="-label"]'
			);
			...
			const fieldGroupCheckboxes = page.locator(
				'input[type="checkbox"][value*="group_"]'
			);
	...
	test.describe( 'PHP Export', () => {
		test( 'should generate PHP code for field group', async ( {
			page,
			admin,
		} ) => {
			...
			const generateButton = page.locator(
				'button[value="generate"]:has-text("Generate PHP")'
			);
			...
			await expect( phpTextarea ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
			...
			const successNotice = page.locator(
				'.acf-admin-notice.notice-success, .notice.notice-success'
			);
		...
		test( 'should include all field settings in PHP export', async ( {
			page,
			admin,
		} ) => {
	...
	test.describe( 'JSON Import', () => {
		test( 'should show error for invalid JSON file', async ( {
			...
			try {
				...
				const importButton = page.locator(
					'button[name="import_type"][value="json"]:has-text("Import JSON")'
				);
		...
		test( 'should show error when no file selected', async ( {
			...
			const warningNotice = page
				...
				.filter( { hasText: 'No file selected' } );
			await expect( warningNotice ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
		} );

		test( 'should show error for empty JSON array', async ( {
			page,
			admin,
		} ) => {
			// Create JSON with valid syntax but no content to import
			const emptyArrayPath = path.join(
				os.tmpdir(),
				'scf-empty-array.json'
			);
		...
		test( 'should update existing field group on import with same key', async ( {
			page,
			admin,
		} ) => {
			...
			try {
				...
				await expect(
					page.locator(
						'.notice.notice-success, .acf-admin-notice.notice-success'
					)
				).toBeVisible();
				...
				const originalCount = await fieldGroups
					.filter( { hasText: 'Original Field Group' } )
					.count();
				const updatedCount = await fieldGroups
					.filter( { hasText: 'Updated Field Group' } )
					.count();
	...
	test.describe( 'Export/Import Round Trip', () => {
		test( 'should preserve field group data through export and re-import', async ( {
			page,
			admin,
		} ) => {
			...
			const groupRow = page.locator(
				`#the-list tr:has-text("${ uniqueGroupName }")`
			);
			...
			const deletedGroupRow = page.locator(
				`#the-list tr:has-text("${ uniqueGroupName }")`
			);
			...
			const importSection = page.locator(
				'.acf-postbox-header:has-text("Import")'
			);
			...
			const importSuccess = page.locator(
				'.notice-success, .updated:has-text("Imported"), .acf-notice.-success'
			);
			await expect( importSuccess.first() ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
			...
			const importedGroupRow = page.locator(
				`#the-list tr:has-text("${ uniqueGroupName }")`
			);
			await expect( importedGroupRow ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );
			...
			const fieldRow = page.locator(
				`.acf-field-object:has-text("${ uniqueFieldLabel }")`
			);
			...
			const labelSetting = fieldRow.locator(
				'.acf-field-setting-label input[type="text"]'
			);
			await labelSetting.waitFor( {
				state: 'visible',
				timeout: DEFAULT_TIMEOUT,
			} );
			...
			const fieldTypeSelect = fieldRow.locator(
				'.acf-field-setting-type select'
			);
		...
		test( 'should preserve post type data through export and re-import', async ( {
			page,
			admin,
		} ) => {
			...
			const postTypeSection = page.locator(
				'.acf-field:has-text("Select Post Types")'
			);
			await expect( postTypeSection ).toBeVisible( {
				timeout: DEFAULT_TIMEOUT,
			} );

			// Find and select our post type
			const ourCheckbox = postTypeSection
				.locator( `label:has-text("${ uniqueName }")` )
			...
			const exportedPt = exportedJson.find(
				( item ) =>
					item.post_type === uniqueSlug || item.title === uniqueName
			);
			...
			await admin.visitAdminPage( 'edit.php', 'post_type=acf-post-type' );
			await expect(
				page.locator( `#the-list tr:has-text("${ uniqueName }")` )
			).not.toBeVisible();
			...
			const importedPluralName = await page
				.locator( '#acf_post_type-labels-name' )
				.inputValue();
			expect( importedPluralName ).toBe( uniqueName );

			// Verify the singular name is preserved
			const importedSingularName = await page
				.locator( '#acf_post_type-labels-singular_name' )
				.inputValue();
		...
		test( 'should preserve taxonomy data through export and re-import', async ( {
			page,
			admin,
		} ) => {
			...
			const taxonomySection = page.locator(
				'.acf-field:has-text("Select Taxonomies")'
			);
			...
			const ourCheckbox = taxonomySection
				.locator( `label:has-text("${ uniqueName }")` )
				.locator( 'input[type="checkbox"]' );
			await ourCheckbox.first().check();
			...
			const exportPath = path.join(
				os.tmpdir(),
				'scf-round-trip-tax-export.json'
			);
			await download.saveAs( exportPath );
			...
			const exportedTax = exportedJson.find(
				( item ) =>
					item.taxonomy === uniqueSlug || item.title === uniqueName
			);
			...
			// Verify the plural name is preserved
			const importedPluralName = await page
				.locator( '#acf_taxonomy-labels-name' )
				.inputValue();
			expect( importedPluralName ).toBe( uniqueName );

			// Verify the singular name is preserved
			const importedSingularName = await page
				.locator( '#acf_taxonomy-labels-singular_name' )
				.inputValue();
		...
		test( 'should preserve options page data through export and re-import', async ( {
			page,
			admin,
		} ) => {
			...
			const optionsSection = page.locator(
				'.acf-field:has-text("Select Options Pages")'
			);
			...
			const ourCheckbox = optionsSection
				.locator( `label:has-text("${ uniqueName }")` )
				.locator( 'input[type="checkbox"]' );
			await ourCheckbox.first().check();
			...
			const exportPath = path.join(
				os.tmpdir(),
				'scf-round-trip-opt-export.json'
			);
			await download.saveAs( exportPath );
			...
			const exportedOpt = exportedJson.find(
				( item ) =>
					item.menu_slug === uniqueSlug || item.title === uniqueName
			);
			...
			const importedPageTitle = await page
				.locator( '#acf_ui_options_page-page_title' )
				.inputValue();
			expect( importedPageTitle ).toBe( uniqueName );

			// Verify the menu slug is preserved
			const importedMenuSlug = await page
				.locator( '#acf_ui_options_page-menu_slug' )
				.inputValue();
	...
	test.describe( 'Import Overwrite', () => {
		test( 'should update existing post type on import with same key', async ( {
			page,
			admin,
		} ) => {
			const uniqueKey =
				'post_type_overwrite_' + Date.now().toString().slice( -6 );
			...
			try {
				...
				await expect(
					page.locator( `#the-list tr:has-text("${ originalName }")` )
				).toBeVisible();
				...
				const originalCount = await postTypes
					.filter( { hasText: originalName } )
					.count();
				const updatedCount = await postTypes
				...
				// Cleanup
				const cleanupRow = page.locator(
					`#the-list tr:has-text("${ updatedName }")`
				);
				if ( await cleanupRow.isVisible().catch( () => false ) ) {
					await cleanupRow
						.locator( 'th.check-column input[type="checkbox"]' )
		...
		test( 'should update existing taxonomy on import with same key', async ( {
			page,
			admin,
		} ) => {
			const uniqueKey =
				'taxonomy_overwrite_' + Date.now().toString().slice( -6 );
			...
			try {
				...
				const originalCount = await taxonomies
					.filter( { hasText: originalName } )
					.count();
				const updatedCount = await taxonomies
					.filter( { hasText: updatedName } )
					.count();
		...
		test( 'should update existing options page on import with same key', async ( {
			page,
			admin,
		} ) => {
			// Key must start with 'ui_options_page_' to be recognized as a valid options page key
			const uniqueKey =
				'ui_options_page_' + Date.now().toString().slice( -8 );
			...
			try {
				...
				const originalCount = await optionsPages
					.filter( { hasText: originalName } )
					.count();
				const updatedCount = await optionsPages
					.filter( { hasText: updatedName } )
					.count();
...
async function cleanupTestEntities( page, admin ) {
	...
	if ( await ptRow.isVisible().catch( () => false ) ) {
		...
		const emptyTrash = page.locator(
			'input[name="delete_all"][value="Empty Trash"]'
		);
	...
	if ( await taxRow.isVisible().catch( () => false ) ) {
		...
		await admin.visitAdminPage(
			'edit.php',
			'post_status=trash&post_type=acf-taxonomy'
		);
		const emptyTrash = page.locator(
			'input[name="delete_all"][value="Empty Trash"]'
		);
