...
const TEST_PLUGIN_SLUG = 'scf-test-plugin-get-field-option-title';
const FIELD_GROUP_LABEL = 'Option Field Group';
const FIELD_LABEL = 'Option Title';
...
test.describe( 'Field Type > Text', () => {
	...
	test( 'should create a text field and verify it in admin', async ( {
		...
		// Adding a micro delay to ensure the option triggers the popup.
		await page.waitForTimeout(50);
		await page.selectOption(
			'select[id^="acf_field_group-location-group_0-rule_0-value"]',
			'add_new_options_page'
		);

		// Wait for the options page popup to be visible
		await page.waitForSelector('#acf-popup[role="dialog"]', { state: 'visible' });

		// Fill the options page form and submit it
		await page.fill('#acf_ui_options_page-page_title', 'Option Fields');
		await page.click('button.acf-btn.acf-btn-primary:has-text("Done")' );
		...
		// Navigate to the created options page
		await admin.visitAdminPage( 'admin.php', 'page=option-fields' );

		// Now you can interact with the options page
		await page.waitForSelector( '.acf-field' );

		// Fill in the Options Title field
		await page.fill(
			'.acf-field[data-name="option_title"] input',
			'My Option Title Value'
		);

		// Click the Update button to save the options
		await page.click( 'input#publish[value="Update"]' );

		// Wait for the page to refresh after update
		...
		// Verify the option field value appears in the post content
 ...
 * Helper function to delete the field group
 */
async function deleteOptionsPages( page, admin ) {
	...
	if ( await allFieldGroupsCheckbox.isVisible() ) {
		...
		await emptyOptionsTrash( page, admin );
	}
}
 ...
 * Helper function to empty trash
 */
async function emptyOptionsTrash( page, admin ) {
	await admin.visitAdminPage(
		'edit.php',
		'post_status=trash&post_type=acf-ui-options-page'
	);
	const emptyTrashButton = page.locator(
		'.tablenav.bottom input[name="delete_all"][value="Empty Trash"]'
	);
